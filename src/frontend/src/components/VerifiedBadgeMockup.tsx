import { CheckCircle, Circle, Clock, ShieldCheck, Star } from "lucide-react";
import type { SellerProfile, VerificationStep } from "../types/blueprint";
          ...
          {verificationProcess.map((step, idx) => {
            ...
            return (
              <div key={step.stepNumber} className="relative flex gap-3">
                ...
                <div
                  className={`w-8 h-8 rounded-full ${cfg.bg} border ${cfg.border} flex items-center justify-center flex-shrink-0 z-10`}
                >
                ...
                <div className="flex-1 pb-1">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <span
                      className={`text-xs font-body px-1.5 py-0.5 rounded-full border ${cfg.bg} ${cfg.color} ${cfg.border}`}
                    >
                      {cfg.label}
                    </span>
                  </div>
        ...
        <div className="flex flex-col items-center text-center gap-3">
          ...
          <div>
            ...
            <span className="text-xs font-body text-muted-foreground">
              Member since {sampleProfile.memberSince}
            </span>
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-sm font-heading font-bold text-foreground ml-1">
              {sampleProfile.rating}
            </span>
          </div>

          <div className="bg-muted/50 rounded-xl px-4 py-2 w-full">
            <span className="text-2xl font-heading font-black text-primary">
              {sampleProfile.transactionCount}
            </span>

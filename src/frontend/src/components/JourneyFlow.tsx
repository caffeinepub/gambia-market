  ...
  return (
    <div className={`rounded-2xl border ${borderClass} p-5`}>
      <div className="flex items-center gap-2 mb-6">
        <span
          className={`px-3 py-1 rounded-full text-sm font-heading font-bold ${accentClass}`}
        >
          {flow.title}
        </span>
      </div>
          ...
          return (
            <div key={step.id} className="flex gap-3">
              ...
              <div className="flex flex-col items-center flex-shrink-0">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-heading font-bold ${numClass} flex-shrink-0`}
                >
                  {step.id}
                </div>
                {!isLast && (
                  <div className={`w-0.5 flex-1 min-h-4 ${lineClass} my-1`} />
                )}
              </div>
              ...
              <div className={`pb-5 ${isLast ? "" : ""}`}>
                <div className="flex items-center gap-2 mb-0.5">
                  <Icon
                    className={`w-4 h-4 ${isGreen ? "text-primary" : "text-secondary"}`}
                  />

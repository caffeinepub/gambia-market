...
import type { BoostTier, ListingComparison } from "../types/blueprint";
...
export default function BoostedListingMockup({
  comparisonDemo,
  pricingTiers,
}: BoostedListingMockupProps) {
  return (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="bg-card border border-border rounded-2xl p-4 shadow-card flex flex-col gap-3">
            ...
            <div>
              <h5 className="font-heading font-bold text-sm text-foreground">
                {comparisonDemo.standard.title}
              </h5>
              <p className="text-primary font-heading font-black text-base">
                D {comparisonDemo.standard.price.toLocaleString()}
              </p>
              <div className="flex items-center gap-1 mt-1">
                <MapPin className="w-3 h-3 text-muted-foreground" />
                <span className="text-xs font-body text-muted-foreground">
                  {comparisonDemo.standard.location}
                </span>
              </div>
            </div>
          ...
          <div className="bg-card border-2 border-accent rounded-2xl p-4 shadow-card ring-2 ring-accent/20 flex flex-col gap-3 relative">
            ...
            <div className="flex items-center justify-between mt-1">
              <span className="text-xs font-body font-semibold text-accent-foreground uppercase tracking-wide">
                Boosted
              </span>
            </div>
            ...
            <div>
              <h5 className="font-heading font-bold text-sm text-foreground">
                {comparisonDemo.boosted.title}
              </h5>
              <p className="text-primary font-heading font-black text-base">
                D {comparisonDemo.boosted.price.toLocaleString()}
              </p>
              <div className="flex items-center gap-1 mt-1">
                <MapPin className="w-3 h-3 text-secondary" />
                <span className="text-xs font-body text-muted-foreground">
                  {comparisonDemo.boosted.location}
                </span>
              </div>
            </div>
            ...
            >
              ...
              <ul className="space-y-1 mt-1">
                {tier.features.map((feat) => (
                  <li
                    key={feat}
                    className="flex items-start gap-1.5 text-xs font-body text-muted-foreground"
                  >

...
import type { FilterOptions } from "../types/blueprint";
    ...
    <div className="mt-5 bg-card border border-border rounded-2xl p-5 shadow-card space-y-4">
      ...
      <div className="flex items-center gap-3 bg-muted/50 border border-border rounded-xl px-4 py-3">
        ...
        <div className="bg-primary rounded-lg px-3 py-1">
          <span className="text-primary-foreground text-xs font-heading font-bold">
            Search
          </span>
        </div>
      </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="bg-muted/50 border border-primary/30 rounded-xl px-3 py-2 flex items-center justify-between">
            <span className="text-sm font-body text-foreground">
              {filterOptions.activeFilters?.category ??
            ...
            {filterOptions.conditions.map((cond) => {
              ...
              return (
                <div key={cond} className="flex items-center gap-2">
                  <div
                    className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 ${isActive ? "bg-primary border-primary" : "border-border bg-card"}`}
                  >
                    {isActive && (
                  ...
                  <span
                    className={`text-sm font-body ${isActive ? "text-foreground font-semibold" : "text-muted-foreground"}`}
                  >
                    {cond}
                  </span>
                </div>

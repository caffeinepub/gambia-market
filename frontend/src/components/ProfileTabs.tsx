  ...
  if (!following || following.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-10 text-center bg-muted/30 rounded-xl border border-dashed border-border">
        ...
        <p className="font-body text-sm text-muted-foreground">Not following anyone yet</p>
        <p className="font-body text-xs text-muted-foreground mt-1">Browse listings and follow sellers you like</p>
      </div>
    );
  }
...
function FollowingSellerCard({ sellerId, onSellerClick }: { sellerId: string; onSellerClick?: (userId: string) => void }) {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          <span className="font-heading font-semibold text-sm text-foreground truncate">{profile.name}</span>
          {profile.verified && (
            <img src="/assets/generated/verified-badge.dim_256x256.png" alt="Verified" className="w-4 h-4 flex-shrink-0" />
          )}
        </div>
        {profile.location && (
          <div className="flex items-center gap-1 text-muted-foreground mt-0.5">
            <MapPin className="w-3 h-3" />
            <span className="text-xs font-body truncate">{profile.location}</span>
          </div>
        )}
      </div>
  ...
  return (
    <Tabs defaultValue="listings" className="w-full">
      <TabsList className="w-full grid grid-cols-4 mx-0 rounded-none border-b border-border bg-transparent h-auto p-0">
        <TabsTrigger
          value="listings"
          className="flex items-center gap-1 py-3 rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:text-primary data-[state=active]:bg-transparent text-muted-foreground font-body text-xs"
        >
      ...
      <TabsContent value="listings" className="px-4 pt-4 mt-0">
        {listingsLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center bg-muted/30 rounded-xl border border-dashed border-border">
            ...
            <p className="font-body text-xs text-muted-foreground mt-1">Tap the Sell tab to create your first listing</p>
          </div>
        ) : (
          <div className="flex flex-col gap-3">
      ...
      <TabsContent value="activity" className="px-4 pt-4 mt-0">
        <div className="flex flex-col items-center justify-center py-10 text-center bg-muted/30 rounded-xl border border-dashed border-border">
          ...
          <p className="font-body text-sm text-muted-foreground">Activity history coming soon</p>
          <p className="font-body text-xs text-muted-foreground mt-1">
            Member since {new Date(Number(profile.createdAt) / 1_000_000).toLocaleDateString()}
          </p>
        </div>
      </TabsContent>
    </Tabs>

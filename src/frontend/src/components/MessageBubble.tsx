...
export default function MessageBubble({ message, isSent }: MessageBubbleProps) {
  ...
  return (
    ...
    >
      <div
        className={`max-w-[75%] ${isSent ? "items-end" : "items-start"} flex flex-col gap-1`}
      >
        {isEditing ? (
          ...
          >
            ...
            {message.isEdited && (
              <span
                className={`text-[10px] ml-1.5 ${isSent ? "text-primary-foreground/60" : "text-muted-foreground"}`}
              >
                (edited)
              </span>
            )}
        ...
        <div
          className={`flex items-center gap-2 ${isSent ? "flex-row-reverse" : "flex-row"}`}
        >

...
import type { CreationStep } from "../types/blueprint";
...
export default function ListingCreationCard({
  creationSteps,
}: ListingCreationCardProps) {
  return (
                ...
                <div className="border-2 border-dashed border-border rounded-xl p-3 flex flex-col items-center gap-1 bg-muted/30">
                  ...
                  <div className="flex gap-1 mt-1">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="w-7 h-7 rounded-md bg-muted border border-border"
                      />
                    ))}
              ...
              {step.fieldType === "radio" && (
                <div className="space-y-1.5">
                  {step.options?.slice(0, 3).map((opt, i) => (
                    <div
                      key={opt}
                      className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-body ${i === 0 ? "border-primary/40 bg-primary/5 text-primary" : "border-border bg-muted/30 text-muted-foreground"}`}
                    >
                      <div
                        className={`w-3 h-3 rounded-full border-2 flex-shrink-0 ${i === 0 ? "border-primary bg-primary" : "border-muted-foreground"}`}
                      />
                      {opt}
                    </div>
                  ))}

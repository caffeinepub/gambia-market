  ...
  return (
    <div className="flex gap-4">
      ...
      <div className="flex flex-col items-center flex-shrink-0">
        <div
          className={`w-10 h-10 rounded-full ${color.badge} flex items-center justify-center font-heading font-black text-sm shadow-md flex-shrink-0`}
        >
          {phase.phase}
        </div>
      ...
      {/* Content */}
      <div
        className={`flex-1 rounded-2xl border ${color.border} ${color.bg} p-5 mb-4`}
      >
            ...
            <ul className="space-y-1.5">
              {phase.deliverables.map((d) => (
                <li key={d} className="flex items-start gap-1.5">
                  <CheckCircle2
                    className={`w-3.5 h-3.5 ${color.num} flex-shrink-0 mt-0.5`}
                  />
                  <span className="text-xs font-body text-foreground leading-snug">
                    {d}
                  </span>
                </li>
              ))}

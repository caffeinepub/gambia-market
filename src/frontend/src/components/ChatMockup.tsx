...
import type { ChatDemo } from "../types/blueprint";
...
export default function ChatMockup({ chatDemo }: ChatMockupProps) {
  return (
    <div className="mt-5 flex justify-center">
      <div
        className="w-full max-w-sm bg-card border border-border rounded-2xl shadow-card overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="bg-muted/60 border-b border-border px-4 py-3 flex items-center gap-3 flex-shrink-0">
          <div
            className={`w-12 h-12 rounded-xl ${chatDemo.listingContext.colorPlaceholder} flex-shrink-0`}
          />
          <div className="flex-1 min-w-0">
            <h4 className="font-heading font-bold text-sm text-foreground truncate">
              {chatDemo.listingContext.title}
            </h4>
            <p className="text-primary font-heading font-black text-sm">
              D {chatDemo.listingContext.price.toLocaleString()}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {chatDemo.sampleMessages.map((msg) => (
            <div
              key={msg.id}
              className={`flex flex-col ${msg.role === "seller" ? "items-end" : "items-start"}`}
            >
              <div

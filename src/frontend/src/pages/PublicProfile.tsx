...
import { ArrowLeft, MapPin, ShieldCheck, Star, Users } from "lucide-react";
...
export default function PublicProfile({
  ...
  const userListings = allListings
    ? allListings.filter((l) => l.sellerId.toString() === userId.toString())
    : [];
        ...
        {listingsLoading ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            {userListings.map((listing) => {
              ...
              const badgeStyle =
                status === "Active"
                  ? { background: "oklch(0.52 0.18 155)", color: "white" }
                  : status === "Sold"
                    ? { background: "oklch(0.55 0.18 220)", color: "white" }
              ...
              return (
                <div key={listing.id.toString()} className="relative">
                  ...
                  {status !== "Active" && (
                    <span
                      className="absolute top-2 left-2 text-[10px] font-body font-bold px-2 py-0.5 rounded-full z-10 pointer-events-none"
                      style={badgeStyle}
                    >
                      {status}
                    </span>

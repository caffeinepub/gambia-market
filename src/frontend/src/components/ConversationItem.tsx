    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-0.5">
          <span
            className={`font-body text-sm truncate ${isUnread ? "font-bold text-foreground" : "font-semibold text-foreground"}`}
          >
        ...
        <p
          className={`text-xs font-body truncate ${isUnread ? "text-foreground font-medium" : "text-muted-foreground"}`}
        >
          {lastMessage}
        </p>
      </div>

      {isUnread && <div className="w-2 h-2 rounded-full shrink-0 bg-primary" />}
    </button>
  );
}

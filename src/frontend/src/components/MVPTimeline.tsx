...
import type { MVPTimelineWeek } from "../types/blueprint";
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 mb-2">
              ...
              />
              <span
                className={`text-xs font-body font-bold ${color.text} uppercase tracking-wide`}
              >
                {week.weeks}
              </span>
            </div>
            ...
            <ul className="space-y-1.5">
              {week.tasks.map((task) => (
                <li key={task} className="flex items-start gap-1.5">
                  <CheckCircle2
                    className={`w-3.5 h-3.5 ${color.text} flex-shrink-0 mt-0.5`}
                  />
